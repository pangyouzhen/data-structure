...
def print_btree(node, node_info=None, inverted=False, is_top=True):
    ...
    left_text_block = [] if not left_node else print_btree(left_node, node_info, inverted, False)

    right_text_block = [] if not right_node else print_btree(right_node, node_info, inverted, False)
